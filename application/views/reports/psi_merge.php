    ...
    <section class="content-header ">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb" style="background-color:transparent ;">
                ...
                <li class="breadcrumb-item"><a href="<?= base_url() ?>index.php/task/psi_revision_viewing" class="btn btn-lg btn-outline-success" style="color:white"><i class="ion-eye"></i> Viewing Mode</a></li>
            </ol>
        </nav>
    </section>
                        ...
                        <div class="box-tools">
                            <div class="input-group">
                                ...
                                <div class="input-group-btn">
                                    <button class="btn btn-lg btn-dark" @click="generatemerge"> <i class="fa fa-search"></i> </button>
                                </div>
                            </div>
                            ...
                            <table class="table table-bordered table-hover " id="tbl_models">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Item Code</th>
                                        <th>Model</th>
                                        <th colspan="3" v-for="months in MonthsInRevision"> {{ months.month_year }} </th>
                                    </tr>
                                    <tr>
                                        <th></th>
                                        <th></th>
                                ...
                                <tbody>
                                    <template v-for="(psi,customer) in psiMerged">
                                        ...
                                        <tr v-for="(psi,models) in psiMerged[customer]">
                                            ...
                                            <td> <b>{{ 1 }}</b> </td>
                                            <template v-for="months in MonthsInRevision">
                                                <td style="text-align:right;"> {{ number_format(psiMerged[customer][models][months.month_year].mergedProdQty / divider) }} </td>
                                                <td style="text-align:right;"> {{ number_format(psiMerged[customer][models][months.month_year].mergedSalesQty / divider) }} </td>
                                                <td style="text-align:right;"> {{ number_format(psiMerged[customer][models][months.month_year].mergedInventory / divider) }} </td>
                                            </template>
                                        </tr>
                                    </template>
...
<script>
    var app = new Vue({
        ...
        methods: {
            generatemerge: function() {
                axios.post("<?= base_url() ?>index.php/task/generatemerge", {
