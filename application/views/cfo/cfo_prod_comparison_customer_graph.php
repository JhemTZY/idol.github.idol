                                                ...
                                                <template v-for="months in month_years">
                                                    <td>
                                                        ...
                                                        <span v-if='total_prod_customer[customersName][month].total_prod_qty!=null'>
                                                            <small> Actual</small>
                                                            <hr>
                                                            {{ total_prod_customer[customersName][month].total_prod_qty }}
                                                        </span>
                                                        <span v-if='total_prod_customer[customersName][month].total_prod_qty==null'>
            ...
            generate_chart: (chartId, prodAmount, prodQuantity) => {
                ...
                for (customer in app.SelectedCustomer) {
                    ...
                    for (month in app.month_years) {
                        ...
                        if (app.total_prod_customer[customerNames][months].total_prod_qty == null) {
                            prod_amount.push(app.total_prod_customer[customerNames][months][app.selectedRevision].total_prod_amount);
                            prod_amount.push(app.total_prod_customer[customerNames][months]['<?= $selectedDPRNO ?>'].total_prod_amount);
                        } else {
                            prod_amount.push(app.total_prod_customer[customerNames][months].total_prod_amount);
                        }
                    }
                ...
                if (app.SelectedCustomer.length < app.customers.length) {
                    for (customer in app.SelectedCustomer) {
                        var prodQty = {};
                        customerNames = app.SelectedCustomer[customer].customers_desc;
                        prodQty['label'] = customerNames + ' QTY';
                        ...
                        for (month in app.month_years) {
                            months = app.month_years[month];
                            if (app.total_prod_customer[customerNames][months].total_prod_qty == null) {
                                prod_quantity.push(app.total_prod_customer[customerNames][months][app.selectedRevision].total_prod_qty);
                                prod_quantity.push(app.total_prod_customer[customerNames][months]['<?= $selectedDPRNO ?>'].total_prod_qty);
                            } else {
                                prod_quantity.push(app.total_prod_customer[customerNames][months].total_prod_qty);
                            }
                        }
