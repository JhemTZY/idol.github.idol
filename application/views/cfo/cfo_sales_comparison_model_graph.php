                                                ...
                                                <template v-for="months in month_years">
                                                    <td>
                                                        ...
                                                        <span v-if='total_sales_customer[customersName][month].total_sales_qty!=null'>
                                                            <small> Actual</small>
                                                            <hr>
                                                            {{ number_format(total_sales_customer[customersName][month].total_sales_qty / divider) }}
                                                        </span>
                                                        <span v-if='total_sales_customer[customersName][month].total_sales_qty==null'>
    ...
    $(document).ready(() => {
        $("#btn_export_sales_comparison_pdf").click(() => {
            ...
            html2canvas(document.querySelector("#cfo_sales_chart")).then(canvas => {
                ...
                doc.save('CFO salesuction Comparison-Graph.pdf');
            });

        });
        $("#btn_export_sales_comparison_excel").click(() => {
            exportTableToExcel("tbl_sales_comparison", "CFO salesuction Comparison");
        });
    })
</script>
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            fetch_salesCompa_graph: () => {
                ...
                axios.post("<?= base_url() ?>index.php/CFO/fetch_sales_model_Compa_Customer_Graph", {
                ...
                if (app.SelectedCustomer.length >= app.SelectedCustomer.length) {
                   ...
                   for (customer in app.SelectedCustomer) {
                    ...
                    for (month in app.month_years) {
                        ...
                        if (app.total_sales_customer[customerNames][modelNames][months].total_sales_qty == null) {
                           
                            sales_amount.push(app.total_sales_customer[customerNames][modelNames][months][app.selectedRevision].total_sales_amount);
                            sales_amount.push(app.total_sales_customer[customerNames][modelNames][months]['<?= $selectedDPRNO ?>'].total_sales_amount);
                        } else {
                            sales_amount.push(app.total_sales_customer[customerNames][modelNames][months].total_sales_amount);
                        }
                    }
                ...
                for (customer in app.SelectedCustomer) {
                    ...
                    for (customer in app.SelectedCustomer) {
                        var salesQty = {};
                         customerNames = app.SelectedCustomer[customer].customers_desc;
                         modelNames = app.SelectedCustomer[customer].models_desc;
                        salesQty['label'] = modelNames + ' QTY';
                        salesQty['borderColor'] = app.SelectedCustomer[customer].customer_color_B;
                        salesQty['backgroundColor'] = app.SelectedCustomer[customer].customer_color_A;
                        salesQty['borderWidth'] = 3;
                         let sales_quantity = [];
                        for (month in app.month_years) {
                            months = app.month_years[month];
                            if (app.total_sales_customer[customerNames][modelNames][months].total_sales_qty == null) {
                                sales_quantity.push(app.total_sales_customer[customerNames][modelNames][months][app.selectedRevision].total_sales_qty);
                                sales_quantity.push(app.total_sales_customer[customerNames][modelNames][months]['<?= $selectedDPRNO ?>'].total_sales_qty);
                            } else {
                                sales_quantity.push(app.total_sales_customer[customerNames][modelNames][months].total_sales_qty);
                            }
                        }
