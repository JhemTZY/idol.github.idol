<div class="content-wrapper" id="psiEdit" @click="PageLoad">
  ...
  <section class="content">
            ...
            <div class="box box-success" style="padding:1rem;">
                <div class="box-header" style="margin-bottom:5rem ;">
                    ...
                    <div class="box-tools">
                        ...
                        <!-- <a class="btn btn-success btn-lg" href="http://10.216.2.202/Datawarehouse_approval/" target="_blank"> <i class="fa fa-envelope"></i> <small> SUBMIT PSI</small> </a> -->
                    ...
                    <tbody>
                        <template v-for="(modelsID,customer) in psiData">
                          <template v-if="(customer ==  'ACTA / B-Company') || (customer ==  'UFF / B-Company') || (customer ==  'CBD / B-Company')">
                            ...
                            <template v-if="Actual !== month.isActual ">
                                ...
                                <template v-if="Actual !== month.isActual ">
                                    <template v-if="models[month?.month_year]?.category==3??'0'">
                                        ...
                                        <!-- Sales -->
                                        <td style="background-color:LightGreen;" v-if="models[month?.month_year]?.isSalesEdit=='1'??'0' "> <input type="text" class="psiCell form-control" style="cursor:cell;  text-align:center; font-size:1.2rem; width:90px;" data-toggle="tooltip" data-placement="right" :title="models[month?.month_year]?.mergedSalesQty??'NULL'"  :placeholder="roundoff(models[month?.month_year]?.mergedSalesQty/1000)??'NULL'" @focus="change_value(two_decimal(models[month?.month_year]?.mergedSalesQty/1000)??'NULL' , $event.target.id)" @blur="one_decimal(roundoff(models[month?.month_year]?.mergedSalesQty/1000)??'NULL' , $event.target.id)" 
                                         @keyup="update_new_plan(models[month?.month_year]?.auto_id, month?.month_year , models[month?.month_year]?.model, models[month?.month_year]?.models_code,  $event.target.value, 'sales_qty', event, $event.target.id)"> </td>
                                         <td style="background-color:LightGreen;" v-if="models[month?.month_year]?.isSalesEdit=='0'??'0' "> <input type="text" class="psiCell form-control" style="cursor:cell; text-align:center; font-size:1.2rem; width:90px;" data-toggle="tooltip" data-placement="right" :title="models[month?.month_year]?.mergedSalesQty??'NULL'"  :placeholder="roundoff(models[month?.month_year]?.mergedSalesQty/1000)??'NULL'" @focus="change_value(two_decimal(models[month?.month_year]?.mergedSalesQty/1000)??'NULL' , $event.target.id)" @blur="one_decimal(roundoff(models[month?.month_year]?.mergedSalesQty/1000)??'NULL' , $event.target.id)" 
                                             @keyup="update_new_plan(models[month?.month_year]?.auto_id, month?.month_year , models[month?.month_year]?.model, models[month?.month_year]?.models_code,  $event.target.value, 'sales_qty', event, $event.target.id)"> </td>
                                             <!-- Sales -->
                                    ...
                                    <template v-if="Actual !== month.isActual ">
                                        ...
                                        <template v-if="Actual !== month.isActual ">
                                            <template v-if="models[month?.month_year]?.category==2??'0'">
                                                ...
                                                <td style="background-color:GoldenRod;" data-toggle="tooltip" data-placement="right" :title="models[month?.month_year]?.mergedSalesQty??'NULL'" > <input type="text" class="psiCell form-control" style="cursor:cell; text-align:center; font-size:1.2rem; width:90px;" :value="roundoff(models[month?.month_year]?.mergedSalesQty/1000??'NULL')" disabled> </td>
                            ...
                            <div class="modal-dialog modal-md modal-dialog-centered">
                              <div class="callout callout-warning">
                                ...
                                <p>Do not Edit (id , model , revision , month year) , Insert New Models and Delete Value based on Template downloads . 
                                    Here's link to add New Model. <a  style="color: skyblue; text-align: center;" href="http://10.216.128.10/Datawarehouse/index.php/User_Maintenance/add_model">PSI MAINTENANCE!</a></p>
                                </div>
                                                    ...
                                                    <div class="box-body">
                                                        <div class="form-group">
                                                            ...
                                                            <form action="" method="get">
                                                                <select class="form-control" style="width: 100%;"  name="txt_selected_month" id="txt_selected_month">
                                        ...
                                        <table class="table table-bordered" id="tbl_PSIupload_Template" hidden>
                                            ...
                                            <tbody>
                                                ...
                                                $models = $this->db->query(
                                                    ...
                                                    WHERE isActual= 'PLAN' AND customer in (3,4,5)  AND revision='$selectedDPRNO' 
    ...
    <!-------------------------------------------------------------------------
                                            ...
                                            <div class="box box-info" style="padding:15px ;">
                                                <div class="box-header">
                                                    <div class="box-body px-5 py-5 ">
                                                        ...
                                                        <form class="px-3 py-3" action="<?php echo base_url(); ?>index.php/Psi_BU/importFile" method="post" enctype="multipart/form-data">
                                                            Upload excel file :
                        ...
                        $action_made = "DOWNLOAD , PSI TEMPLATE";

                        $get_data = $this->psim->UserLog($user_id,  $role_text, $name, $action_made); ?>
    ...
    // Set Cell in Local Storage
                    ...
                    var app = new Vue({
                        ...
                        methods: {
                            GeneratePSIedit: function() {
                                axios.post("<?= base_url() ?>index.php/Psi_BU/GeneratePSIedit", {
                ...
                // Update only when these keys are used | Reduce Server Usage
                                if (updateTriggerKeys.includes(e.keyCode) || !isNaN(e.key)) {
                                    axios.post("<?= base_url() ?>index.php/Psi_BU/update_new_plan", {
                                        SelectedRevision: this.current_revision,
                                        id: id,
                                        month: month,
                                        model: model,
                ...
                // Generate the PSI | Event: Press Enter
                            ...
                            GetRevisionData: function() {
                                axios.post("<?= base_url() ?>index.php/Psi_BU/GetCurrentRevisionData", {
                                    SelectedRevision: this.current_revision,
                                }).then((response) => {
                    // console.log(response.data);
